...
using EforTakip.Application.WorkLogs.Queries.GetWorkLogs;
...
public sealed class WorkLogsController(ISender mediator) : ControllerBase
{
    [RequirePermission(Permissions.WorkLog.Create)]
    [HttpPost]
    ...
    [HttpGet]
    [ProducesResponseType(typeof(PagedResult<WorkLogDto>), StatusCodes.Status200OK)]
    public async Task<ActionResult<PagedResult<WorkLogDto>>> GetAll(
        [FromQuery] GetWorkLogsQuery query, CancellationToken cancellationToken)
    ...
    [RequirePermission(Permissions.WorkLog.Delete)]
    [HttpDelete("{id:guid}")]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
