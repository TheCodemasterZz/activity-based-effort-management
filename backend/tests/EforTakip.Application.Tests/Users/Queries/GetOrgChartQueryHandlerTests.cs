using EforTakip.Application.Users.Queries.GetOrgChart;
...
namespace EforTakip.Application.Tests.Users.Queries;
...
public sealed class GetOrgChartQueryHandlerTests : IAsyncDisposable
{
    ...
    public async Task Handle_WithManagerMapping_ResolvesManagerId()
    {
        ...
        var manager = User.CreateFromActiveDirectory(
            directory.Id, "gokhan.yetkin", "Gökhan", "Yetkin", "Gökhan Yetkin", null, "guid-manager");
        var employee = User.CreateFromActiveDirectory(
        ...
        _db.Users.AddRange(manager, employee);
        await _db.SaveChangesAsync();
    ...
    public async Task Handle_WithManagerOutsideSyncScope_ExposesUnresolvedManagerName()
    {
        ...
        // girmediği için sistemde hiç User kaydı yok — ManagerId null kalır ama frontend'in
        ...
        employee.SetAttribute(mapping.Id, "Gökhan Yetkin", referencedUserId: null);
